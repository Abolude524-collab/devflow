...
import { acceptInvitation, declineInvitation, getUserNotifications, InvitationError, sendProjectInvite, } from './invitation.service.js';
function getUserId(request) {
    return request.user.sub;
}
export async function inviteUserController(request, reply) {
    try {
        await request.jwtVerify();
        const { projectId } = projectParamsSchema.parse(request.params);
        const body = inviteUserBodySchema.safeParse(request.body);
        if (!body.success)
            return reply.code(400).send({ message: 'Enter a valid user email address' });
...
export async function getUserNotificationsController(request, reply) {
    try {
        await request.jwtVerify();
...
export async function acceptInvitationController(request, reply) {
    try {
        await request.jwtVerify();
...
export async function declineInvitationController(request, reply) {
    try {
        await request.jwtVerify();
        const { notificationId } = notificationParamsSchema.parse(request.params);
