...
import { ChatError, getProjectMessages, saveChatMessage } from './chat.service.js';
...
export async function uploadChatAttachmentController(request: FastifyRequest, reply: FastifyReply) {
  try {
    await request.jwtVerify();
    const { projectId } = projectParamsSchema.parse(request.params);
