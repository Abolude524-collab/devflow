...
import { ChatAttachment, ChatMessageModel } from './chat.model.js';

export class ChatError extends Error {}

async function assertProjectAccess(projectId: string, userId: string) {
  if (!isValidObjectId(projectId)) throw new ChatError('Project not found');
  const project = await ProjectModel.findById(projectId);
  if (!project) throw new ChatError('Project not found');

  const isWorkspaceMember = await WorkspaceModel.exists({ _id: project.workspaceId, 'members.userId': userId });
  if (!isWorkspaceMember) throw new ChatError('Access denied');
...
export async function saveChatMessage(
  ...
  if (!user) throw new ChatError('User not found');
...
export async function getProjectMessages(projectId: string, userId: string, channelId?: string) {
