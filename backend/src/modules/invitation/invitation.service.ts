...
export async function sendProjectInvite(projectId: string, inviterId: string, inviteeEmail: string) {
  if (!isValidObjectId(projectId)) throw new InvitationError('Project not found');
  const project = await ProjectModel.findById(projectId);
  if (!project) throw new InvitationError('Project not found');
  ...
  if (!isWorkspaceMember) throw new InvitationError('Workspace access denied');

  const inviter = await UserModel.findById(inviterId);
  if (!inviter) throw new InvitationError('Inviter not found');
  ...
  const isProjectMember =
    String(project.ownerId) === String(inviteeUser._id) ||
...
export async function acceptInvitation(notificationId: string, userId: string) {
  if (!isValidObjectId(notificationId)) throw new InvitationError('Notification not found');
  const notification = await NotificationModel.findOne({ _id: notificationId, userId });
  if (!notification) throw new InvitationError('Notification not found');
  ...
  await ProjectModel.updateOne(
    { _id: invitation.projectId, 'members.userId': { $ne: userId } },
    { $push: { members: { userId, role: 'member' } } },
  );
...
export async function declineInvitation(notificationId: string, userId: string) {
  if (!isValidObjectId(notificationId)) throw new InvitationError('Notification not found');
  const notification = await NotificationModel.findOne({ _id: notificationId, userId });
  if (!notification) throw new InvitationError('Notification not found');
