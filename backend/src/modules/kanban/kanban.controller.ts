...
export async function getProjectBoardController(request: FastifyRequest, reply: FastifyReply) {
  try {
    await request.jwtVerify();
    const { projectId } = projectParamsSchema.parse(request.params);
    return reply.send(await getProjectBoard(projectId, getUserId(request)));
  } catch (error) {
    if (error instanceof KanbanNotFoundError) return reply.code(404).send({ message: error.message });
    if (error instanceof KanbanAccessError) return reply.code(403).send({ message: error.message });
...
export async function createTaskController(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    if (!body.success) return reply.code(400).send({ message: 'Invalid task data', errors: body.error.flatten() });
...
export async function moveTaskController(request: FastifyRequest, reply: FastifyReply) {
  try {
    await request.jwtVerify();
    const { taskId } = taskParamsSchema.parse(request.params);
    const body = moveTaskBodySchema.safeParse(request.body);
    if (!body.success) return reply.code(400).send({ message: 'Invalid target column data' });
...
export async function updateTaskController(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    if (!body.success) return reply.code(400).send({ message: 'Invalid task update data' });
...
export async function addCommentController(request: FastifyRequest, reply: FastifyReply) {
  try {
    await request.jwtVerify();
    const { taskId } = taskParamsSchema.parse(request.params);
    const body = addCommentBodySchema.safeParse(request.body);
    if (!body.success) return reply.code(400).send({ message: 'Comment text is required' });
